...
async def on_message(message):
    ...
    if message.author.id == '150577293981515776':
        ...
        if message.content == "/pn f" or message.content == "/pn full":
            ...
            embed = discord.Embed(description="\n\n"
                                                              ...
                                                              "\n이제 ``/게임 주사위`` 명령어를 통해 점수를 획득할 수 있습니다.\n``/게임 가위바위보``명령어를 통한 점수 변동이 기존 1점에서 10점으로 변경하였습니다.\n처음 점수 설정이 50점에서 500점으로 변경하였습니다.\n\n"
                                                              "```1.1.2.0000 (r:28-190207)```\n``/게임`` 명령어를 추가하였습니다.\n\n"
                                                              "```1.1.2.0001 (r:29-190207)```\n``/게임 주사위`` 명령어의 재사용 대기 안내 메시지의 시간 표기를 개선하였습니다.\n\n"
                                                              "```1.1.2.0002 (r:30-190208)```\n이제 금지어 사용 시 출력되는 경고 메시지에 채널이 기록됩니다.\n\n"
                                                              "```1.1.2.0003 (r:31-190208)```\n7개의 금지어를 추가하였습니다.\n\n"
                                                              "```1.1.2.0004 (r:32-190208)```\n77개의 금지어를 삭제하였습니다.\n일부 명령어를 사용하면 봇이 금지어로 감지하는 문제점을 해결하였습니다.\n\n"
                                                              "```1.1.2.0005 (r:33-190208)```\n이제 ``/ㅂㅅㄱ`` 명령어를 ``/ㅄㄱ``으로도 사용할 수 있습니다.\n\n"
                                                              "```1.1.2.0006 (r:34-190208)```\n``/게임 주사위`` 명령어의 재사용 대기시간을 180초에서 150초로 변경하였습니다.\n\n"
                                                              "```1.1.2.0007 (r:35-190209)```\n이제 금지어 사용 시 출력되는 경고 메시지는 금지어가 사용 된 채널에 전송되지 않으며 경고는 <@150577293981515776>의 확인 및 판단 후에 누적됩니다.\n\n"
                                                              "```1.1.2.0008 (r:36-190209)```\n금지어 사용 시 출력되는 경고 메시지에 반응이 추가되도록 변경하였습니다.\n\n"
                                                              "```1.1.2.0009 (r:37-190209)```\n2개의 금지어를 추가하였습니다.\n\n"
                                                              "```1.1.2.0010 (r:38-190209)```\n2개의 금지어를 추가하였습니다.\n\n"
                                                              , color=0xffff00)
            await client.send_message(message.channel, embed=embed)
            embed = discord.Embed(description="\n\n"
                                                              "```1.2.0.0000 (r:39-190209)```\n이제 봇이 새벽 시간대에 꺼지는 대신 Sleep 상태 적용되어 24시간 가동됩니다.\nSleep 상태에서는 일부 명령어를 사용할 수 없습니다.\n\n"
                                                              , color=0xffff00)
            await client.send_message(message.channel, embed=embed)
            embed = discord.Embed(description="\n\n"
                                                              "```1.3.0.0000 (r:40-190223)```\n``.vote1``, ``.vote2``, ``.daytime``, ``.night`` 명령어를 추가하였습니다.\n\n"
                                                              "```1.3.1.0000 (r:41-190224)```\n``.argument`` 명령어를 추가하였습니다.\n이제 ``.vote2`` 명령어에 반응이 추가되지 않습니다.\n\n"
                                                              "```1.3.1.0001 (r:42-190224)```\n일부 채널에서 금지어 감지 기능을 비활성화하였습니다.\n\n"
                                                              , color=0xffff00)
            await client.send_message(message.channel, embed=embed)
    ...
    "후뢰" in message.content:
        if message.channel.id != '548548456436990003' and message.channel.id != '548548289222410241' and message.channel.id != '548548318830133278':
